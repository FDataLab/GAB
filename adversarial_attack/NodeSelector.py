...
class NodeSelector:
    ...
    def _filter_node_minimum_degree(
        margin_dict: Dict, min_degree: int, data: Data
    ) -> Dict:
        """
        ...
        valid_node = [
            node for node, degree in enumerate(degree_values) if degree > min_degree
        ]
        valid_margin_dict = {}
    ...
    def select_nodes_margin(
        ...
        if min_degree > 0:
            margin_dict = NodeSelector._filter_node_minimum_degree(
                margin_dict, min_degree, data
            )
        ...
        return {"high_margin": high, "low_margin": low, "other": other}
    ...
    def select_nodes_margin_degree(
        ...
        highest_degree, lowest_degree = NodeSelector._select_node_value_margin(
            degree_values
        )
    ...
    def _calculate_degrees(adj_matrix, node_list: List):
        ...
        ).flatten()  # Sum of rows (or columns, as it's symmetric)
