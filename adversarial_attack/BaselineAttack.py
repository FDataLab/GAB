...
class RandomDegreeAttack(RandomAttack):
    ...
    def __init__(self, data, device = "cpu", seed = None, name = None, r = 0.5,**kwargs):
        ...
        self.deg = degree(data.edge_index[1], num_nodes=data.num_nodes, dtype=torch.long).cpu().numpy()
        self.r = r
    ...
    def get_added_edge(self, influence_nodes):
        ...
        attacker_nodes = random.sample(attacker_nodes,int(len(attacker_nodes) * self.r))
        best_node = None
        best_degree = -1
...
class RandomOnlyAddAttack(RandomDegreeAttack):
    ...
    """
    def attack(self, target, *, num_budgets=None, threshold=1,
               direct_attack=True, structure_attack=True, feature_attack=False,
               disable=False):

        TargetedAttacker.attack(self= self,target=target, target_label=None, num_budgets=num_budgets,
...
class DegreeAttack:
    ...
    def attack(self, target,num_budgets=None,structure_attack = True, feature_attack = False):
