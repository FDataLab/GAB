...
class PRBCDAttack(torch.nn.Module):
    ...
    *e.g.*, :class:`~torch_geometric.nn.conv.GCNConv` or
    ...
    def data(self):
        assert self.modified_edgelist is not None, "attack() method must be called before calling data()"
        modified_data = copy.deepcopy(self.ori_data)
        modified_data.edge_index = self.modified_edgelist
        return modified_data

    def update_config(self,block_size: int,
        epochs: int = 125,
    ...
    def attack(
        ...
        **kwargs,
        ...
        Args:
            ...
            target (torch.Tensor, optional): Filter for predictions/labels.
        ...
        assert self.model is not None, "setup_surrogate() must be called before calling attack"
        self.model.eval()
        self.log = verbose
        assert kwargs.get('edge_weight') is None
        edge_weight = torch.ones(self.edge_index.size(1), device=self.device)
        if num_budgets is None:
            num_budgets = self.num_budgets
        self.edge_index = self.edge_index.cpu().clone()
        ...
        step_sequence = self._prepare(num_budgets)
        target = [target]
        ...
        for step in tqdm(step_sequence, disable=not self.log, desc='Attack'):
            loss, gradient = self._forward_and_gradient(
                self.x, self.labels, target, **kwargs)

            scalars = self._update(step, gradient, self.x, self.labels, num_budgets,
                                   target, **kwargs)
        ...
        perturbed_edge_index, flipped_edges = self._close(
            self.x, self.labels, num_budgets, target, **kwargs)

        assert flipped_edges.size(1) <= num_budgets, (
            f'# perturbed edges {flipped_edges.size(1)} '
            f'exceeds budget {num_budgets}')

        self.modified_edgelist =perturbed_edge_index
        self.flip_edge = flipped_edges
...
class GRBCDAttack(PRBCDAttack):
    ...
    def __init__(
       ...
       seed=720
        ...
        super().__init__(data,device,seed)
