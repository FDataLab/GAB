...
if __name__ == "__main__":
    ...
    target_bucket = NodeSelector.select_nodes_margin_degree(
        surrogate_supervisor, data, idx_test, seed=args.seed
    )

    adversarial_surrogate = ConfigHandler.load_model_config(
        data, idx_train, idx_val, device=args.device, seed=args.seed, model="GCN"
    )
    nettack = Nettack(data, args.device, args.seed)
    nettack.setup_surrogate(adversarial_surrogate)

    adv_supervisor = AdversarialSupervisor(
        data, nettack, idx_train, idx_val, idx_test, args.device, **best_config
    )
    (
        high_result,
        low_result,
        highest_degree_result,
        lowest_degree_result,
        other_result,
    ) = adv_supervisor.target_attack(
        target_bucket,
        5,
        evasion=True,
        poision=True,
        evasion_model=surrogate_supervisor,
        seed=720,
    )
    evasion_count = (
        high_result[EVASION]
        + high_result[EVASION]
        + low_result[EVASION]
        + highest_degree_result[EVASION]
        + lowest_degree_result[EVASION]
        + other_result[EVASION]
    )
    poision_count = (
        high_result[POSION]
        + high_result[EVASION]
        + low_result[POSION]
        + highest_degree_result[POSION]
        + lowest_degree_result[POSION]
        + other_result[POSION]
    )
