if __name__ == "__main__":
    ...
    from utility.util import init_logger, prepare_dir, set_random

    args.dataset = "cora"
    args.model = "GCN"
    init_logger(
        prepare_dir(args.output_folder)
        + args.model
        + "_"
        + args.dataset
        + "_seed_"
    ...
    config = ConfigHandler.load_model_config(
        model_type=args.model, split=0, config_name="best_config", dataset=args.dataset
    )
    ...
    idx_train, idx_val, idx_test = (
        split_config["idx_train"],
        split_config["idx_val"],
        split_config["idx_test"],
    )
