...
class OrbitTableGenerator:
    ...
    def generate_orbit_table(self):
        if self.dataset in ["cora", "citeseer", "polblogs", "pubmed"]:
            ...
            if os.path.exists(
                "{}{}_orbit_table.csv".format(self.filepath, self.dataset)
            ):
                return pd.read_csv(
                    "{}{}_orbit_table.csv".format(self.filepath, self.dataset)
                )
    ...
    def generate_orbit_tables_from_sratch(self):
        ...
        df_2d = pd.DataFrame(
            my_array,
            columns=["node_number", "Orbit_type_I", "Orbit_type_II", "two_Orbit_type"],
        )
...
def generate_orbit_tables_from_count(data_list, nodes_list):
    ...
    for i in range(len(graphlet_features)):
        ...
        mylist.append(
            [nodes_list[i], str(sorted_indices[0]), str(sorted_indices[1]), s1]
        )
