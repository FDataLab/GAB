...
__all__ = ["singleton_filter", "SingletonFilter", "LikelihoodFilter", "singleton_mask"]
...
def singleton_mask(adj_matrix: Tensor):
    ...
    mask = 1.0 - logical_and_symmetric.float()
    return mask
...
class LikelihoodFilterTensor:
    ...
    def update_Sx(S_old, n_old, d_old, d_new, d_min):
        ...
        new_S_d = (
            S_old
            - torch.log(torch.maximum(d_old_in_range, 1)).sum(1)
            + torch.log(torch.maximum(d_new_in_range, 1)).sum(1)
        )
