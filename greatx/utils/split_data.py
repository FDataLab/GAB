...
def split_nodes(
    ...
    *,
    ...
    return BunchDict(
        dict(train_nodes=train_nodes, val_nodes=val_nodes, test_nodes=test_nodes)
    )


def split_nodes_by_classes(
    labels: torch.Tensor, n_per_class: int = 20, random_state: Optional[int] = None
) -> BunchDict:
    ...
    return BunchDict(
        dict(train_nodes=split_train, val_nodes=split_val, test_nodes=split_test)
    )
