...
class MissingFeature(BaseTransform):
    ...
    * https://github.com/twitter-research/feature-propagation
    ...
    def __call__(self, data: Data) -> Data:
        ...
        if (
            self.missing_type == "structural"
        ):  # either remove all of a nodes features or none
            missing_mask = (
                torch.bernoulli(torch.Tensor([self.missing_rate]).repeat(num_nodes))
                .bool()
                .unsqueeze(1)
            ...
            missing_mask = torch.bernoulli(
                torch.Tensor([self.missing_rate]).repeat(num_nodes, num_features)
            ).bool()
