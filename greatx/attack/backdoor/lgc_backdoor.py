...
class LGCBackdoor(BackdoorAttacker):
    ...
    * Please remember to call :meth:`reset` before each attack.
    ...
    def attack(
        self, num_budgets: Union[int, float], target_class: int, disable: bool = False
    ) -> "LGCBackdoor":
        ...
        trigger[feat_perturbations] = 1.0
    ...
    @staticmethod
    def get_feat_perturbations(
        W: Tensor, target_class: int, num_budgets: int
    ) -> Tensor:
...
class FGBackdoor(BackdoorAttacker, Surrogate):
    ...
    * Please remember to call :meth:`reset` before each attack.
    ...
    def setup_surrogate(
        self, surrogate: nn.Module, *, tau: float = 1.0
    ) -> "FGBackdoor":
    ...
    def attack(
        ...
        ) = get_backdoor_edges(self.edge_index, N)

        for _ in tqdm(
            range(self.num_budgets),
            ...
            gradients = torch.autograd.grad(-loss, trigger)[0] * (1.0 - trigger)
...
def get_backdoor_edges(edge_index: Tensor, N: int) -> Tuple:
    ...
    d_augmented = d.clone()
    d_augmented[influence_nodes] += 1.0
    d_augmented = torch.cat(
        [d_augmented, torch.full(trigger_nodes.size(), 2, device=device)]
    )
    ...
    edge_weight = (
        d_pow[edge_index_with_self_loop[0]] * d_pow[edge_index_with_self_loop[1]]
    )
    edge_weight_with_trigger = d_augmented_pow[edge_index[0]] * d_pow[edge_index[1]]
    trigger_edge_weight = (
        d_augmented_pow[trigger_edge_index[0]] * d_augmented_pow[trigger_edge_index[1]]
    )
