...
class Nettack(TargetedAttacker, Surrogate):
    ...
    * Please remember to call :meth:`reset` before each attack.
    ...
    def compute_cooccurrence_constraint(self, nodes):
        ...
        words_graph = self.cooc_matrix - sp.diags(
            self.cooc_matrix.diagonal(), format="csr"
        )
    ...
    def compute_logits(self):
        return (self.adj_norm @ self.adj_norm @ self.modified_feat @ self.W)[
            self.target
        ].ravel()
    ...
    def feature_scores(self):
        ...
        gradient = self.gradient_wrt_x(self.target_label) - self.gradient_wrt_x(
            best_wrong_class
        )
    ...
    def get_attacker_nodes(self, n=5, add_additional_nodes=False):
        ...
        assert (
            n < self.modified_adj.shape[0] - 1
        ), "number of influencers cannot be >= number of nodes!"
        neighbors = self.modified_adj[self.target].indices
        candidate_edges = np.column_stack(
            (np.tile(self.target, len(neighbors)), neighbors)
        ).astype("int32")
        ...
        if len(neighbors) >= n:
            ...
            if add_additional_nodes:
                ...
                poss_add_infl = np.setdiff1d(
                    np.setdiff1d(np.arange(self.modified_adj.shape[0]), neighbors),
                    self.target,
                )
                n_possible_additional = len(poss_add_infl)
                n_additional_attackers = n - len(neighbors)
                possible_edges = np.column_stack(
                    (np.tile(self.target, n_possible_additional), poss_add_infl)
                ).astype("int32")
        ...
        *,
        ...
        if ll_constraint:
            likelihood_filter = LikelihoodFilter(
                self.degree.cpu().numpy(), ll_cutoff=ll_cutoff
            )

        for it in tqdm(
            range(self.num_budgets), desc="Perturbing graph...", disable=disable
        ):
            ...
            if structure_attack:
                ...
                if ll_constraint:
                    candidate_edges = likelihood_filter(
                        candidate_edges,
                        edge_weights=self.modified_adj[
                            candidate_edges[:, 0], candidate_edges[:, 1]
                        ].A1,
                    )
    ...
    def compute_new_a_hat_uv(
        ...
        for ix in range(len(candidate_edges)):
            ...
            for v in affected_nodes:
                ...
                mult_term = 1.0 / np.sqrt(degs_new[u] * degs_new[v])
