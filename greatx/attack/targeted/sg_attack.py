    ...
    * Please remember to call :meth:`reset` before each attack.
    ...
    def setup_surrogate(
        ...
        *,
        ...
        self.logits = self.surrogate(self.feat, self.edge_index, self.edge_weight).cpu()
    ...
    def get_subgraph(self, target, target_label, best_wrong_label):
        ...
        if self.direct_attack:
            ...
            num_attackers = 3
        attacker_nodes = self.get_top_attackers(
            subgraph,
            target,
            target_label,
            best_wrong_label,
    ...
    def get_top_attackers(
        self, subgraph, target, target_label, best_wrong_label, num_attackers
    ):
        non_edge_grad, _ = self.compute_gradients(
            subgraph, target, target_label, best_wrong_label
        )
        _, index = torch.topk(
            non_edge_grad, k=min(num_attackers, non_edge_grad.size(0)), sorted=False
        )
    ...
    def subgraph_processing(self, sub_nodes, sub_edges, influencers, attacker_nodes):
        ...
        non_edge_weight = torch.zeros(
            non_edges.size(1), device=self.device
        ).requires_grad_()
        ...
        *,
        ...
        for it in tqdm(
            ...
            if max_edge_grad > max_non_edge_grad:
                # remove one edge
                subgraph.edge_weight.data[max_edge_idx].fill_(0.0)
                u, v = subgraph.sub_edges[:, max_edge_idx].tolist()
                self.remove_edge(u, v, it)
            else:
                # add one edge
                subgraph.non_edge_weight.data[max_non_edge_idx].fill_(1.0)
    ...
    def compute_gradients(self, subgraph, target, target_label, best_wrong_label):
        edge_weight = torch.cat(
            [
                subgraph.non_edge_weight,
                subgraph.edge_weight,
                subgraph.non_edge_weight,
                subgraph.edge_weight,
        ...
        return grad(
            loss, [subgraph.non_edge_weight, subgraph.edge_weight], create_graph=False
        )
