...
from torch_geometric.nn.models.basic_gnn import GAT, GIN, PNA, EdgeCNN, GraphSAGE

from greatx.defense.purification import SVDPurification
from greatx.nn.models import GNNGUARD, SGC, ElasticGNN, RobustGCN
...
from defense_model.improving_architecture.aggregation.soft_median import (
...
def load_model(in_channel: int, out_channel: int, degree_dist: torch.Tensor, **kwargs):
    ...
    if model_name == "GCN":
        ...
        model = GCN(in_channel, nhid, nlayers, out_channel, drop_out, aggr=aggr)
    elif model_name == "GIN":
        model = GIN(in_channel, nhid, nlayers, out_channel, drop_out, aggr=aggr)
    elif model_name == "GSAGE":
        model = GraphSAGE(in_channel, nhid, nlayers, out_channel, drop_out, aggr=aggr)
    elif model_name == "PNA":
        ...
        model = GAT(in_channel, nhid, nlayers, out_channel, drop_out, aggr=aggr)
    elif model_name == "EdgeCNN":
        model = EdgeCNN(in_channel, nhid, nlayers, out_channel, drop_out, aggr=aggr)
    elif model_name == "SGC":
        model = SGC(
        ...
        model = Noisy_GCN(
            nfeat=in_channel, nhid=nhid, nclass=out_channel, dropout=drop_out
        )
...
def load_graph_purification(device: Optional[str] = None, **kwargs):
    ...
    if purification_name == "Jaccard":
        ...
        gamma = kwargs.get(GAMMA)
        purification = GarnetPurification(
            r,
            k,
            gamma,
