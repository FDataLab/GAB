...
from .utils import gcn_norm_fill_val, get_rw_adj


class ConstantODEblock_ENERGY(ODEblock):
    def __init__(self, odefunc, opt, device, t=torch.tensor([0, 1])):
        super(ConstantODEblock_ENERGY, self).__init__(odefunc, opt, device, t)
    ...
    def forward(self, x, adj):
        ...
        if self.opt["adjoint"] and self.training:
            state_dt = integrator(
                ...
                adjoint_options=dict(
                    step_size=self.opt["adjoint_step_size"],
        ...
        #   np.save('state_save'+ self.opt['function']+'.npy', state_save)
        ...
        if not self.training:
            with torch.no_grad():
                for i in range(len(state_dt)):
                    print("i: ", i)
                    if self.opt["function"] == "hang":
                        energy = self.odefunc.H(
                            state_dt[i].detach().clone(), self.odefunc.edge_index
                        )
                    elif self.opt["function"] == "hangquad":
                        energy = self.odefunc.H(
                            state_dt[i].detach().clone(),
                            self.odefunc.edge_index,
                            self.odefunc.edge_weight,
                        )
                    else:
                    ...
                    np.save(
                        "energy_list_attack" + self.opt["function"] + ".npy",
                        energy_list,
                    )
                # energy = self.odefunc.H(x, self.odefunc.edge_index)
                # print("energy: ", energy)
