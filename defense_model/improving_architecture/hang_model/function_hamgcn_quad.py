...
from torch_geometric.utils import degree, get_laplacian, softmax, to_dense_adj
...
class HAMGCNFunc_QUAD(ODEFunc):
    ...
    def forward(self, t, x_full):  # the t param is needed by the ODE solver.
        ...
        f_full = batch_jacobian(
            lambda xx: self.H(xx, self.edge_index, self.edge_weight),
            x_full,
            create_graph=True,
        ).squeeze()
        ...
        if self.opt["add_source"]:
            dx = (1.0 - torch.sigmoid(self.beta_train)) * dx + torch.sigmoid(
                self.beta_train
