...
class ODEFuncAttNorm(ODEFunc):
    ...
    def __init__(self, in_features, out_features, opt, device):
        ...
        self.multihead_att_layer = SpGraphAttentionLayer(
            in_features, out_features, opt, device
        ).to(device)
        try:
        ...
        assert (
            self.attention_dim % opt["heads"] == 0
        ), "Number of heads must be a factor of the dimension size"
        self.d_k = self.attention_dim // opt["heads"]

        self.multihead_att_layer_norm = DeepGATConv(
            in_features, out_features, heads=opt["heads"], concat=False, dropout=0.3
        ).to(device)
            ...
            wx = torch.mean(
                torch.stack(
                    [
                        ...
                        for idx in range(self.opt["heads"])
                    ],
                    dim=0,
                ),
                dim=0,
    ...
    def forward(self, t, x_full):  # t is needed when called by the integrator
        ...
        if self.opt["add_source"]:
            f = (1.0 - torch.sigmoid(self.beta_train)) * f + torch.sigmoid(
                self.beta_train
            ) * self.x0[:, self.opt["hidden_dim"] :]
        f = torch.cat(
            [
                f,
                (1.0 - torch.sigmoid(self.beta_train2)) * alpha * x
                + torch.sigmoid(self.beta_train2)
                * self.x0[:, : self.opt["hidden_dim"]],
            ],
            dim=1,
        )
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, opt, device, concat=True):
        ...
        self.W = nn.Parameter(torch.zeros(size=(in_features, self.attention_dim))).to(
            device
        )
        nn.init.xavier_normal_(self.W.data, gain=1.414)

        self.Wout = nn.Parameter(
            torch.zeros(size=(self.attention_dim, self.in_features))
        ).to(device)
    ...
    def forward(self, x, edge):
        ...
        edge_h = (
            torch.cat((h[edge[0, :], :, :], h[edge[1, :], :, :]), dim=1)
            .transpose(0, 1)
            .to(self.device)
        )  # edge: 2*D x E
...
class DeepGATConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.norm = LipschitzNorm(
            scale_individually=False
        )  # normalization method: {lipschitznorm, neighbornorm, pairnorm, None}
    ...
    def message(
        ...
        if self.norm is not None:
            alpha = self.norm(
                x_j, att=(self.att_l, self.att_r), alpha=alpha, index=index
            )
...
class LipschitzNorm(nn.Module):
    ...
    def forward(self, x, att, alpha, index):
        ...
        max_norm = torch.sqrt(
            max_norm[index] + norm_x
        )  # simulation of max_j ||x_j||^2 + ||x_i||^2
        ...
        if self.scale_individually == False:
            ...
            norm_att = self.att_norm * torch.norm(
                torch.cat((att_l, att_r), dim=-1), dim=-1
            )
