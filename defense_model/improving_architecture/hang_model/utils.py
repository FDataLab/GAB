...
def gcn_norm_fill_val(
    edge_index, edge_weight=None, fill_value=0.0, num_nodes=None, dtype=None
):
...
def get_rw_adj_old(data, opt):
    if opt["self_loop_weight"] > 0:
        edge_index, edge_weight = add_remaining_self_loops(
            data.edge_index, data.edge_attr, fill_value=opt["self_loop_weight"]
        )
    else:
