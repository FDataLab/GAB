...
class ODEFuncBeltramiAtt(ODEFunc):
    ...
    def __init__(self, in_features, out_features, opt, device):
        ...
        self.multihead_att_layer = SpGraphTransAttentionLayer(
            in_features, out_features, opt, device, edge_weights=self.edge_weight
        ).to(device)
        self.device = device
    ...
    def multiply_attention(self, x, attention, v=None):
        ...
        if mix_features:
            ...
            grad_x_norm = torch.sqrt(
                torch.sum(torch.clamp(grad_x_abs * grad_x_abs, min=1e-1), 1)
            )
            ...
            W = torch.sparse.FloatTensor(
                self.edge_index, new_attn, (x.shape[0], x.shape[0])
            ).coalesce()
            rowsum = torch.sparse.mm(
                W, torch.ones((W.shape[0], 1), device=self.device)
            ).flatten()
            diag_index = torch.stack(
                (torch.arange(x.shape[0]), torch.arange(x.shape[0]))
            ).to(self.device)
...
class SpGraphTransAttentionLayer(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, opt, device, concat=True, edge_weights=None
    ):
...
if __name__ == "__main__":
    ...
    t = 1
    func = ODEFuncTransformerAtt(
        dataset.data.num_features, 6, opt, dataset.data, device
    )
