...
from defense_model.utils import load_graph_purification, load_model
from evaluation.utils import load_opt, load_scheduler
from static import *
from utility.config import args
from utility.util import logger, set_random
...
class ModelSupervisor:
    ...
    def __init__(
        ...
        **model_args
        ...
        if opti_state is not None:
            self.optimizer = load_opt(model_args.get(OPTIMIZER))(
        ...
        if self.scheduler is not None:
            ...
            self.scheduler = load_scheduler(self.scheduler)(
                self.optimizer, **scheduler_state
            )
        ...
        if use_purification:
            assert (
                purification_config is not None
            ), "In order to use purification, please define purification config"
            self.purification = load_graph_purification(
                device=self.device, **purification_config
            )
    ...
    def _train(self, idx_train: List[int]) -> float:
        ...
        if self.model_name != "GRAND":
            ...
            loss = self.model(
                self.features_torch, self.edge_index_torch, idx_train, self.labels_torch
            )
        loss.backward()
        self.optimizer.step()
        if self.scheduler is not None:
    ...
    def train_model(
        ...
        """
        logger.info(
            ">>Start traing {} on {} for node classification".format(
                args.model, args.dataset
            )
        )
        logger.info("Number of nodes:{}".format(self.number_nodes))
        logger.info(
            "Training node:{} , Validation node: {} , Test node: {}".format(
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            # Validation and test
            epoch_output = self.get_model_output(
                self.data, force_no_use_purification=True
            )  # force: to avoid keep using purification
            epoch_prediction = epoch_output.argmax(dim=-1)
            val_loss = F.cross_entropy(
                epoch_output[idx_val], self.labels_torch[idx_val]
            )
            train_acc = self.get_prediction_accuracy(
                idx_train, self.data, epoch_prediction
            )
            test_acc = self.get_prediction_accuracy(
                idx_test, self.data, epoch_prediction
            )
            ...
            if epoch == 1 or epoch % log_interval == 0:
                logger.info("==" * 30)
                logger.info(
                    "Epoch:{}, Loss:{:.4f}, Time:{:.3f}, GPU:{:.1f}MiB".format(
                        epoch, loss, delta_epoch, gpu_mem_alloc
                    )
                )
                logger.info(
                    "Train: {:.3f}, Validation accuracy:{:.3f}, Test: {:.3f}".format(
        ...
        logger.info(
            "Best val:{:.3f}, Best test: {:.3f}".format(best_val_acc, best_test_acc)
        )

    def get_model_output(
        self, data: Data, force_no_use_purification: bool = False
    ) -> torch.Tensor:
        """
        ...
        if self.use_purification and not force_no_use_purification:
            assert (
                self.purification is not None
            ), "In order to use purification, please define purification config when initialize the supervisor"
            purification = load_graph_purification(
                device=self.device, **self.purification_config
            )
    ...
    def get_prediction_accuracy(
        self, mask, data: Data, prediction: bool = None
    ) -> float:
        """
